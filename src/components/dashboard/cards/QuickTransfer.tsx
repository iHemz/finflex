    ...
    <section className="md:col-span-4 card p-4 flex flex-col justify-between">
      ...
      <div className="flex">
        {transactions.slice(0, 3).map((transaction) => (
          <button
            key={transaction.id}
            className="cursor-pointer w-12 h-12 lg:w-20 lg:h-20 rounded-full bg-gray-800 object-cover overflow-hidden"
          >
            <img src={transaction.logo} alt="" className="object-cover" />
          </button>
        ))}
        <button className="cursor-pointer hover:scale-105 w-12 h-12 lg:w-20 lg:h-20 rounded-full bg-gray-800 object-cover overflow-hidden transition-all">
          +
        </button>
      </div>
    </section>
